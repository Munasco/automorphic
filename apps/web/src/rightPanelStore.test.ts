...
describe("rightPanelStore", () => {
  it("opens Trading alongside Diff and restores Diff when Trading closes", () => {
    const store = useRightPanelStore.getState();
    store.open(refA, "diff");
    store.open(refA, "trading");
    store.open(refA, "trading");
    const current = () =>
      selectThreadRightPanelState(useRightPanelStore.getState().byThreadKey, refA);
    expect(current().surfaces.map((surface) => surface.kind)).toEqual(["diff", "trading"]);
    expect(current().activeSurfaceId).toBe("trading");
    store.closeSurface(refA, "trading");
    expect(current().activeSurfaceId).toBe("diff");
    store.open(refA, "trading");
    expect(current().activeSurfaceId).toBe("trading");
    expect(
...
it("drops obsolete PR tabs while retaining Files and Diff", () => {
  const migrated = migratePersistedRightPanelState({
    byThreadKey: {
      "env-1:thread-A": {
        isOpen: true,
        activeSurfaceId: "pull-request:old",
        surfaces: [
          { id: "pull-request:old", kind: "pull-request" },

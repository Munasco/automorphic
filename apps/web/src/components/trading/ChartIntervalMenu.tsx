  ...
  return (
    <Menu>
      ...
      <MenuPopup
        align="end"
        sideOffset={0}
        className="w-36 min-w-0 max-h-[min(var(--available-height),28rem)] overflow-y-auto rounded-none"
      >
        {[favoritesGroup, ...groups]
          .filter((group) => group.items.length)
          .map(({ group, items }) => (
            <MenuGroup key={group} aria-label={group}>
              <MenuGroupLabel className="px-2 py-0.5 text-[10px]">{group}</MenuGroupLabel>
              {items.map((item) => {
                ...
                return (
                  <div key={item.key} className="group/interval flex items-center">
                    <MenuItem
                      ...
                      className="h-6 min-h-6 min-w-0 flex-1 gap-1.5 rounded-none px-2 py-0 text-xs sm:min-h-6 sm:text-xs"
                    ...
                    <MenuItem
                      ...
                      closeOnClick={false}
                      className="h-6 min-h-6 w-6 shrink-0 justify-center sm:min-h-6 rounded-none px-0 opacity-0 group-hover/interval:opacity-100 group-focus-within/interval:opacity-100 data-highlighted:opacity-100 [@media(hover:none)]:opacity-100"

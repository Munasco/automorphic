...
export interface HollowCandlePalette {
  up: string;
  down: string;
  wickUp?: string;
  wickDown?: string;
...
export function hollowCandleColors(
  bar: Candle,
  previous?: Candle,
  palette: HollowCandlePalette = { up: "#26a69a", down: "#ef5350" },
): HollowCandleColors {
  ...
  const up = bar.close >= previousClose;
  const directionColor = up ? palette.up : palette.down;
  return {
    color: bar.close > bar.open ? "transparent" : directionColor,
    borderColor: (up ? palette.borderUp : palette.borderDown) ?? directionColor,
    wickColor: (up ? palette.wickUp : palette.wickDown) ?? directionColor,
  };
}

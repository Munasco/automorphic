...
export const INDICATOR_DEFINITIONS = [
  ...
  defineIndicator({
    ...
    detail: "Moving average with configurable range bands",
    category: "Overlays",
    placement: "overlay",
    inputs: [
      ...
      priceSource,
      {
        ...
        step: 1,
        options: [
          { value: 0, label: "Average true range" },
          { value: 1, label: "True range" },
          { value: 2, label: "Smoothed high-low" },
        ],
      },
      length(10, "atrPeriod", "ATR length"),
      { key: "multiplier", label: "Band multiplier", defaultValue: 2, min: 0, max: 20, step: 0.1 },
    ],
    styles: [
    ...
    calculate: ({ bars, inputs }) =>
      bands(
        calculateKeltnerChannels(
          ...
          inputs.rangeType === 1 ? "trueRange" : inputs.rangeType === 2 ? "highLow" : "atr",
        ),
        bars,
      ),
  }),

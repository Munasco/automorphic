...
export function renderChartReport(
  ...
  const levels = input.levels
    .map(
      (level) =>
        `<g><title>${escape(level.label)} · ${price(level.price)}</title><line x1="${left}" x2="${right}" y1="${y(level.price)}" y2="${y(level.price)}" stroke="#ededed" stroke-dasharray="5 5"/><text x="${right - 6}" y="${y(level.price) - 6}" text-anchor="end" class="level">${escape(level.label)} · ${price(level.price)}</text></g>`,
    )
    .join("");
  const annotations = input.annotations
    .map(
      (a, index) =>
        `<a href="#note-${index + 1}" aria-label="${escape(a.label)}"><circle cx="${x(indices.get(a.time)!)}" cy="${y(a.price)}" r="12" fill="#ededed" stroke="#0a0a0a" stroke-width="2"/><text x="${x(indices.get(a.time)!)}" y="${y(a.price) + 4}" text-anchor="middle" fill="#0a0a0a" font-size="11" font-weight="700">${index + 1}</text><title>${escape(a.label)}</title></a>`,
    )
    .join("");
...
${TRADING_REPORT_CSS}
.summary{white-space:pre-wrap}.chart{background:var(--panel);border:1px solid var(--border);border-radius:8px;padding:16px 8px;overflow:auto;margin:24px 0 12px}svg{display:block;width:100%;min-width:720px}.grid{stroke:var(--grid);stroke-width:1}.level{fill:var(--text);font:12px var(--font);paint-order:stroke;stroke:var(--panel);stroke-width:4px;stroke-linejoin:round}svg g:focus>rect:first-of-type{fill:#ffffff10}.notes{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(240px,100%),1fr));gap:16px;margin:24px 0}.notes article{padding:20px;border:1px solid var(--border);border-radius:8px;scroll-margin-top:24px}.notes article:target{border-color:var(--muted)}article p{margin:8px 0;white-space:pre-wrap;color:var(--muted)}details{margin:24px 0}.table{overflow:auto}@media print{.chart,article{break-inside:avoid}}

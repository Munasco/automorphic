...
it.effect("defaults application data to .automorphic while preserving explicit dev homes", () =>
  Effect.gen(function* () {
    const path = yield* Path.Path;
    assert.equal(yield* resolveBaseDir(undefined), path.join(NodeOS.homedir(), ".automorphic"));
    assert.equal(yield* resolveBaseDir("/tmp/automorphic-dev/.t3"), "/tmp/automorphic-dev/.t3");

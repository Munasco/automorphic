...
const DESKTOP_APP_ID = "com.automorphic.desktop";
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.string("AUTOMORPHIC_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
  });
  const rawRepo = (Option.getOrUndefined(env.updateRepository)?.trim() || "").trim();
  // A fork must never inherit the upstream feed from CI or legacy T3 settings.
  if (!rawRepo || /^(pingdotgg|t3tools)\/t3code$/i.test(rawRepo)) return undefined;
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Automorphic (Nightly)"
    : (desktopPackageJson.productName ?? "Automorphic");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "Automorphic-${version}-${arch}.${ext}",
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSScreenCaptureUsageDescription:
          "Automorphic captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "T3 Code",
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Automorphic trading workspace",
    author: "Automorphic",
